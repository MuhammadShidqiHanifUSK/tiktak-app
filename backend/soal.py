...
def get_soal():
  ...
  cursor = conn.cursor()
  cursor.execute(
    'SELECT * FROM soal WHERE tingkat_kesulitan = ? ORDER BY RANDOM() LIMIT 1',
    (tingkat,)
  )
  row = cursor.fetchone()
  conn.close()
...
# Tambah soal baru + generate audio TTS
...
def add_soal():
  ...
  audio_filename = f"soal_{jawaban_jam}_{jawaban_menit}_{tingkat_kesulitan}.mp3"
  ...
  cursor = conn.cursor()
  cursor.execute(
    'INSERT INTO soal (cerita, jawaban_jam, jawaban_menit, tingkat_kesulitan, audio_path) VALUES (?, ?, ?, ?, ?)',
    (cerita, jawaban_jam, jawaban_menit, tingkat_kesulitan, audio_filename)
  )
  conn.commit()
